...
const Payment = () => {
    ...
    console.log(orderId);
    const { data: item, isLoading } = useQuery('orderFind', () => (fetch(`https://fair-gold-bull-tam.cyclic.app/order/payment/${orderId}`, {
        method: 'GET',
        headers: {
        ...
        <div className="px-2 pt-20 md:px-20 bg-[url('https://images.unsplash.com/photo-1563013544-824ae1b704d3?crop=entropy&cs=tinysrgb&fm=jpg&ixlib=rb-1.2.1&q=60&raw_url=true&ixid=MnwxMjA3fDB8MHxzZWFyY2h8Mnx8cGF5fGVufDB8fDB8fA%3D%3D&auto=format&fit=crop&w=500')] bg-no-repeat bg-cover rounded">
            <div className='flex flex-col md:flex-row justify-evenly items-center h-screen md:gap-28'>
                ...
                <div className="card bg-white w-full shadow-xl">
                    <div className="p-4">
                        <h2 className="text-2xl">Hello  <span className='text-purple-600'>{user?.displayName || 'My dear customer'}</span></h2>
                        <p>your <span className='text-purple-500'>{item.payload[0].partName} </span> is ready to pay.</p>
                        <p>pay $<span className='text-primary'>{item.payload[0].price}</span></p>
                ...
                <div className="card w-full bg-base-100 shadow-2xl">
                    <div className='card-body'>
                        <Elements stripe={stripePromise}>
                            <CheckoutForm item={item.payload[0]} ></CheckoutForm>
                        </Elements>
                    </div>
                </div>
            </div>
