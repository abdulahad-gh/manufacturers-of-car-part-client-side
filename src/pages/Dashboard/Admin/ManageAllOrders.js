...
const ManageAllOrders = () => {
    const { data: allOrders, isLoading, refetch } = useQuery('getAllOrders', () => fetch('http://localhost:5000/all-orders', {
        method: 'GET',
        headers: {
    ...
    const handlePendingStatus = id => {
        fetch(`http://localhost:5000/pending-status-change/${id}`, {
            method: 'PUT',
            headers: {
                authorization: `Bearer ${localStorage.getItem('accessToken')}`
            }
                            ...
                            {allOrders?.map((order, i) => <>
                                <tr key={i}>
                                    ...
                                    {(order.price && !order.paid) ? <td>
                                        <label for="delete-doctor" className="btn btn-xs btn-outline btn-error">Delete</label>
