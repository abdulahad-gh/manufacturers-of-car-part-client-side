...
const ManageAllOrders = () => {
    const { data: allOrders, isLoading, refetch } = useQuery('getAllOrders', () => fetch('https://manufacturers-of-car-part-server.up.railway.app/all-orders', {
        method: 'GET',
        headers: {
    ...
    const handlePendingStatus = id => {
        fetch(`https://manufacturers-of-car-part-server.up.railway.app/pending-status-change/${id}`, {
            method: 'PUT',
            headers: {
                authorization: `Bearer ${localStorage.getItem('accessToken')}`
            }
