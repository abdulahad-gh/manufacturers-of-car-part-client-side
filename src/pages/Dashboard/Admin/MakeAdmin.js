...
const MakeAdmin = () => {
    ...
    const { data: users, refetch } = useQuery('findAllUser', () => fetch('https://manufacturers-of-car-part-server-production.up.railway.app/users', {
        headers: {
    ...
    const makeAdmin = (email) => {
        setLoading(true)
        fetch(`https://manufacturers-of-car-part-server-production.up.railway.app/user/admin/${email}`, {
            method: 'PUT',
            headers: {
                'authorization': `Bearer ${localStorage.getItem('accessToken')}`
            }
